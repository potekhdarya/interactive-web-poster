...
const MOCK_SESSIONS = [
  {
    id: 'RZSMARO',
    date: '27.02.2026',
    analysis: { Тревожность: 'средняя', 'Характер движения': 'беспокойный' },
    points: generateMockPoints(420, W, H, 'anxious')
  },
  {
    id: 'PWCGTUY',
    date: '21.01.2026',
    analysis: { Тревожность: 'высокая', 'Характер движения': 'хаотичный' },
    points: generateMockPoints(680, W, H, 'chaotic')
  },
  {
    id: 'SJKNPXE',
    date: '15.01.2026',
    analysis: { Тревожность: 'средняя', 'Характер движения': 'активный' },
    points: generateMockPoints(510, W, H, 'anxious')
  },
  {
    id: 'BKRMVWZ',
    date: '10.01.2026',
    analysis: { Тревожность: 'средняя', 'Характер движения': 'неуверенный' },
    points: generateMockPoints(390, W, H, 'anxious')
  },
  {
    id: 'QTLXFPD',
    date: '05.01.2026',
    analysis: { Тревожность: 'низкая', 'Характер движения': 'спокойный' },
    points: generateMockPoints(280, W, H, 'calm')
  },
  {
    id: 'HNVYCRS',
    date: '01.01.2026',
    analysis: { Тревожность: 'низкая', 'Характер движения': 'осторожный' },
    points: generateMockPoints(310, W, H, 'calm')
  },
  {
    id: GLITCH_ID,
    date: '28.12.2025',
    analysis: { Тревожность: 'неизвестна', 'Характер движения': 'нелинейный' },
...
function getTag(analysis) {
  const anxiety = (analysis && analysis['Тревожность']) || '';
  const char = (analysis && analysis['Характер движения']) || '';
  if (anxiety === 'высокая')
    return { label: 'Крайне тревожен', cls: 'journalCardTag--critical' };
  if (anxiety === 'средняя')
    return { label: 'Тревожен', cls: 'journalCardTag--anxious' };
  if (char === 'спокойный' || char === 'осторожный')
    return { label: 'Стабилен', cls: 'journalCardTag--stable' };
  return { label: 'Иное', cls: 'journalCardTag--other' };
}

function filterGroup(tag) {
  const l = tag.label.toLowerCase();
  if (l.includes('тревож')) return 'тревожные';
  if (l.includes('стабил')) return 'стабильные';
  return 'иные';
}
...
function loadJournal() {
  ...
  if (countEl) countEl.textContent = `Всего записей: ${allSessions.length}`;
  ...
  document.querySelectorAll('.journalFilterBtn').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      btn.style.color = '#fff';
      renderCards(btn.dataset.filter, allSessions);
    });
  });

  renderCards('all', allSessions);
}

function renderCards(filter, sessions) {
  const grid = document.getElementById('journalGrid');
  if (!grid) return;
  grid.innerHTML = '';
  ...
  filtered.forEach((session) => {
    ...
    const isLocal = session._local === true;

    const card = document.createElement('div');
    card.className = 'journalCard';
    if (isLocal) card.classList.add('journalCard--local');
